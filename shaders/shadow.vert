...
void main() {
    ...
    vec4 ShadowCoord = biasMatrix*dp*dv*dm * vec4(position,1);

    float bias = 0.005;
    float visibility = 1.0;

    for (int i=0;i<4;i++){
      if ( texture( tex, ShadowCoord.xy + poissonDisk[i]/700.0 ).z  <  ShadowCoord.z-bias ){
        visibility-=0.2;
      }
    }
    ...
    for (int i = 0; i < MAX_LIGHTS; i++) {
        ...
        color += visibility * max(vec3(0), lightColors[i] * diffuse_color * diffuseIntensity)*d;

        // Add specular component
        vec4 lightReflection = normalize(-reflect(vertexToLight, normal_cameraSpace));
        vec4 eyeDirection = normalize(vec4(0,0,0,1) - position_cameraSpace);
        float specIntensity = pow(max(0.0, dot(eyeDirection, lightReflection)), shininess);
        color += visibility * max (vec3(0), lightColors[i] * specular_color * specIntensity)*s;
    }
